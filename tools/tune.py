...
class SiamIRCATracker(SiameseTracker):
    def __init__(self, model):
        super(SiamIRCATracker, self).__init__()
        self.score_size = (cfg.TRACK.INSTANCE_SIZE - cfg.TRACK.EXEMPLAR_SIZE) // \
            cfg.POINT.STRIDE + 1 + cfg.TRACK.BASE_SIZE
        hanning = np.hanning(self.score_size)
        window = np.outer(hanning, hanning)
        self.cls_out_channels = cfg.IRCA.KWARGS.cls_out_channels
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='tuning for SiamIRCA')
